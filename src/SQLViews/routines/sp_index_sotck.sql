...
SELECT DISTINCT
    ...
    pp.cantidad_bandejas_disponibles                            AS cantidadBandejasDisponibles,
    (pp.cantidad_bandejas_disponibles +
     COALESCE(br.cantidad_bandejas_reservadas, 0))              AS stockTotal,
    tb.nombre                                                   AS tipoBandeja,
    pp.fecha_siembra_pedido                                     AS fechaSiembraPedido,
    pp.fecha_entrega_pedido                                     AS fechaEntregaPedido,
    pp.fecha_entrega_pedido_real                                AS fechaEntregaPedidoReal,
...
FROM pedido p
         ...
         LEFT JOIN _view_bandejas_reservadas_por_producto br ON br.id = pp.id

...
function datatablesGetColDef() {
    ...
    return [
        ...
        },
        {
            ...
            render: function (data, type, full, meta) {
                if (type === 'display') {
                    return '<span class="label label-inline margin-0 font-weight-bold p-6" style="width: 220px;color: #000000 !important;background-color: ' + data.colorProducto + '">' + data.nombreProductoCompleto + '</span>';
                }
...
function dataTablesActionFormatter(data, type, full, meta) {
    ...
    if (jQuery.isEmptyObject(data)) {
        ...
        actions +=
            ...
            +
            (data.delete !== undcanceefined ? '<a class="dropdown-item accion-borrar" href="' + data.delete + '"><i class="la la-remove" style="margin-right: 5px;"></i> Borrar</a>' : '')
            +
